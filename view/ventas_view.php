...
$id_almacen = $_GET['id_almacen'] ?? 1;
$_GET['id_almacen'] = $id_almacen;
...
$action = 'almacenes';
$almacenes = include(__DIR__ . '/../php/ventas.php');
?>
  ...
  <div class="row mb-4 align-items-center">
    ...
    <div class="col-md-4">
      <input type="text" id="buscador" class="form-control"
             placeholder="Buscar producto por modelo...">
    </div>

    <div class="col-md-4 d-flex gap-2">
      <select class="form-select"
        onchange="location.href='index.php?vista=ventas&id_almacen='+this.value">
        <?php foreach ($almacenes as $a): ?>
          <option value="<?= $a['id_almacen'] ?>"
            <?= $id_almacen == $a['id_almacen'] ? 'selected' : '' ?>>
  ...
  <div class="row g-4">
    ...
    <div class="col-md-8">
      ...
      <div class="row g-3" id="contenedor-productos">
        ...
        <?php $modeloEscapado = htmlspecialchars($row['modelo'], ENT_QUOTES, 'UTF-8'); ?>

        <div class="col-md-3 tarjeta-producto"
             data-modelo="<?= strtolower($modeloEscapado) ?>">
        ...
        <ul class="pagination justify-content-center">
          <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
            <li class="page-item <?= $pagina_actual == $i ? 'active' : '' ?>">
              <a class="page-link"
                 href="index.php?vista=ventas&p=<?= $i ?>&id_almacen=<?= $id_almacen ?>">
                <?= $i ?>
              </a>
            </li>
          <?php endfor; ?>
    ...
    <div class="col-md-4">
      ...
      <div class="card shadow border-0 p-3 mb-5"
           style="position: sticky; top: 20px; background:#fff;">

        <h4 class="text-secondary border-bottom pb-2 fw-bold">
          🛒 Carrito de Compras
        </h4>
        ...
        <button class="btn btn-success w-100 mt-3 fw-bold py-2"
